			...
			if(!file.isDirectory())
			{
				...
				while(readFile.hasNextLine()) 
				{
					...
					if(currLine.trim().contains("STRING"))
					{
						tempFile.write(currLine+"\n");
						currLine = readFile.nextLine();
						if(currLine.trim().contains("SELECT") || currLine.trim().contains("INSERT ") || currLine.trim().contains("UPDATE ") || currLine.trim().contains("DELETE "))
						{
							tempFile.write(currLine+"\n");
						...
						else
						{
							...
							{	
								while(m2.find())
								{
										if(m2.group(1).trim().length()>2 && !m2.group(1).trim().equals("YYYYMMDD") && !m2.group(1).trim().equals(oldApplicationName) && !fileNamesList.containsValue(m2.group(1).trim()))
										{
							...
							{	
								while(m.find())
								{
										if(m.group(1).trim().length()>2 && !m.group(1).trim().equals("YYYYMMDD") && !m.group(1).trim().equals(oldApplicationName) && !fileNamesList.containsValue(m.group(1).trim()))
										{
											String randomString = generateRandomString(m.group(1));
											currLine = currLine.replace(m.group(1), randomString);	
										}
